module.exports = {
    name: 'promote',
    description: 'Promotes a tagged or quoted person to admin.',
    category: 'Group',
    async execute(conn, chatId, args, ownerId, senderId, isAdmin, quotedUser) {
        ...
        if (!targetUser) {
            await conn.sendMessage(chatId, { 
                text: `@${senderId.split('@')[0]}, please tag or quote the user you want to promote.`,
                mentions: [senderId]
            });
            return;
        }
        ...
        try {
            ...
            await conn.sendMessage(chatId, { 
                text: `@${targetUser.split('@')[0]} has been promoted to admin by @${senderId.split('@')[0]}.`,
                mentions: [targetUser, senderId]
            });
        } catch (error) {
            console.error('Error promoting user:', error);
            await conn.sendMessage(chatId, {
                text: `@${senderId.split('@')[0]}, I couldn't promote the user. Ensure I have admin rights.`,
                mentions: [senderId]
            });
        }
    }
